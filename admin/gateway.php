...
if (isset($_POST["salvar_rushpay"])) {
    $secret_key = $_POST["rushpay_secret_key"];
    $public_key = $_POST["rushpay_public_key"];

    $stmt = $pdo->prepare("UPDATE rushpay SET secret_key = ?, public_key = ? WHERE id = 1");
    if ($stmt->execute([$secret_key, $public_key])) {
        $_SESSION["success"] = "Credenciais RushPay alteradas!";
    } else {
        $_SESSION["failure"] = "Erro ao alterar as credenciais RushPay!";
    }
    header("Location: ".$_SERVER["PHP_SELF"]);
    exit;
}
...
// Buscar dados RushPay
$stmt = $pdo->query("SELECT secret_key, public_key FROM rushpay WHERE id = 1");
$rushpay = $stmt->fetch(PDO::FETCH_ASSOC);
if (!$rushpay) {
    $rushpay = ["secret_key" => "", "public_key" => ""];
}
                ...
                <div class="gateway-navbar">
                    ...
                    <form method="POST" class="gateway-switch-form">
                        <select name="gateway_ativa" class="gateway-select" onchange="this.form.submit()">
                            ...
                            <option value="rushpay" <?= ($gateway["active"] == "rushpay") ? "selected" : "" ?>>
                                RushPay
                            </option>
                ...
                <div class="form-container">
                    <form method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="rushpay_secret_key" class="form-input" 
                                   value="<?= htmlspecialchars($rushpay['secret_key'] ?? '') ?>" 
                                   placeholder="213d1905-9ac0-4023-8dbd-0279918c7bcd" required>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="rushpay_public_key" class="form-input" 
                                   value="<?= htmlspecialchars($rushpay['public_key'] ?? '') ?>" 
                                   placeholder="50742ec4-8eac-4516-a957-b896209ce27c" required>
                        </div>

                        <button type="submit" name="salvar_rushpay" class="submit-button">
                            <i class="fas fa-bolt"></i>
                            Salvar Credenciais RushPay
                        </button>
                    </form>
                </div>
